 ...
 */
class BoxSlotlet implements ISlotlet
{
  ...
  {
    return array('frontend/slotlet/sl_box.css');
  }
...
SLOTLET;
  ...
  public function getConfigurationForm($values = array())
  {
    ...
    $form .= strtr($row, array(
      ...
      '%field%' => input_tag('description', $values['description'], array('class' => 'slotlet_option'))
    ));
  ...
  protected function renderContent($shortcut, $options)
  {
    ...
    $template = '<div class="shortcut %type% %with_image% %is_external%">%imagen% %title%</div><div class="description">%body%</div>';

    return $shortcut->renderBoxSlotlet(strtr($template, array(
      ...
      '%body%'        => $shortcut->getBody(),
      '%imagen%'      => $shortcut->getMultimedia() ? $shortcut->getMultimedia()->getHTMLRepresentation() : null
    )));
  }
