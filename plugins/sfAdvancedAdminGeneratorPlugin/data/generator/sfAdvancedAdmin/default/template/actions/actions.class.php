...
class <?php echo $this->getGeneratedModuleName() ?>Actions extends sfActions
{
  ...
  {
<?php if ($bt = $this->getParameterValue('belongs_to.class', '')): ?>
    $this->belongs_to = <?php echo $bt ?>Peer::retrieveByPk($this->getUser()->getAttribute('<?php echo sfInflector::underscore($bt) ?>_pk'));
    $this->forward404Unless($this->belongs_to);
<?php endif ?>
  ...
  public function executeAutocomplete() {
    ...
    $search = $this->getRequestParameter(strtolower("${table}_${field}_search"));
    $return = '';
    $c = new Criteria();
...
<?php if ($bt = $this->getParameterValue('belongs_to.class', '')): ?>
    ...
    $this-><?php echo $this->getSingularName() ?>->set<?php echo sfInflector::camelize($bt) ?>($this->belongs_to);
<?php endif ?>
  }
...
<?php endif; ?>
<?php if ($bt = $this->getParameterValue('belongs_to.class', '')): ?>
    $c->add(<?php echo $this->getPeerClassName() ?>::<?php echo strtoupper(sfInflector::underscore($bt)) ?>_ID, $this->belongs_to->getId());
<?php endif ?>
  }
...
<?php if ($bt = $this->getParameterValue('belongs_to.class')): ?>
  ...
  {
<?php $module = $this->getParameterValue('belongs_to.module')?$this->getParameterValue('belongs_to.module'):$this->getBelongsToClassName(); ?>
<?php $action = $this->getParameterValue('belongs_to.action')?$this->getParameterValue('belongs_to.action'):'index' ?>
    $this->getRequest()->setParameter('id', $this->belongs_to->getPrimaryKey());
    $this->forward('<?php echo $module ?>', '<?php echo $action ?>');
  }
<?php endif ?>

<?php $object_actions = $this->getParameterValue('list.object_actions') ?>
<?php $edit_actions = $this->getParameterValue('edit.actions') ?>
<?php $show_actions = $this->getParameterValue('show.actions') ?>
<?php if ($object_actions || $edit_actions || $show_actions): ?>
<?php $arr = array() ?>
<?php if (is_array($object_actions)): ?>
<?php $arr = array_merge($arr, $object_actions) ?>
<?php endif ?>
<?php if (is_array($edit_actions)): ?>
<?php $arr = array_merge($arr, $edit_actions) ?>
<?php endif ?>
<?php if (is_array($show_actions)): ?>
<?php $arr = array_merge($arr, $show_actions) ?>
<?php endif ?>
<?php foreach ($arr as $key => $value): ?>
<?php if (substr($key, 0, 10) == '_has_many_'): ?>
  public function executeList<?php echo sfInflector::camelize($has_many = substr($key, 10, -1)) ?>s()
  {
    $this->getUser()->setAttribute('<?php echo sfInflector::underscore($this->getClassName()) ?>_pk', $this->get<?php echo $this->getClassName() ?>OrCreate()->getPrimaryKey());
<?php $module = (isset($value['module']))?$value['module']:strtolower($has_many) ?>
<?php $action = (isset($value['action']))?$value['action']:'index' ?>
    $this->forward('<?php echo $module ?>', '<?php echo $action ?>');
  }
