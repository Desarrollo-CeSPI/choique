...
<?php foreach ($this->getColumns('create.display', $category) as $name => $column): ?>
<?php if ($column->getPhpName() != $this->getParameterValue('belongs_to.class').'Id'): ?>
...
<?php endif; ?>
<div class="form-row">
  [?php echo label_for('<?php echo $this->getParameterValue("create.fields.".$column->getName().".label_for", $this->getSingularName()."[".$column->getName()."]") ?>', __($labels['<?php echo $this->getSingularName() ?>{<?php echo $column->getName() ?>}']), '<?php if ($column->isNotNull()): ?>class="required" <?php endif; ?>') ?]
