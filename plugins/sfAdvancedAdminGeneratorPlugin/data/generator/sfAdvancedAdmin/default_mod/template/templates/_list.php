...
<tr>
<?php if ($this->getParameterValue('list.batch_actions')): ?>
  <th id="sf_admin_list_th_sf_batch_actions">
    [?php echo checkbox_tag('foo', 1, 0, array('onclick' => "boxes = document.getElementsByTagName('input'); for(index in boxes) { box = boxes[index]; if (box.type == 'checkbox' && box.className == 'sf_admin_batch_checkbox') box.checked = this.checked } return true;")) ?]
  </th>
<?php endif; ?>
...
[?php include_partial('list_td_<?php echo $this->getParameterValue('list.layout', 'tabular') ?>', array('<?php echo $this->getSingularName() ?>' => $<?php echo $this->getSingularName() ?>, 'maps' => $maps)) ?]
