...
<?php echo input_hidden_tag('article[multimedia_id]', $article->getMultimediaId()) ?>
<?php echo input_auto_complete_tag('multimedia_id_search',
                                   ($article->getMultimediaId()) ? $article->getMultimedia()->__toString() : '',
                                   'article/autocompleteMultimedia?_csrf_token='.csrf_token(),
                                   array('size' => '80'),
                                   array('use_style' => true,
                                         'after_update_element' => "function(inputField, selectedItem) {" .
                                                                   "$('article_multimedia_id').value = selectedItem.id; }",
                                         'indicator' => 'searching')) ?>
